...
describe("useEngine", () =>
{
    it("serves the same sheet for the same character, versions and language", () =>
    {
        ...
        const first = engine.sheet(CLERIC, [SRD], "en");

        expect(engine.sheet(structuredClone(CLERIC), [SRD], "en")).toBe(first);
    ...
    it("recomputes the sheet, not the package set, when the character or the language changes", () =>
    {
        ...
        const renamed = engine.sheet({ ...CLERIC, name: "Another cleric" }, [SRD], "en");
        const italian = engine.sheet(CLERIC, [SRD], "it");
    ...
    {
        const engine = useEngine();
        engine.sheet(CLERIC, [SRD], "en");

        const newer = { ...SRD, manifest: { ...SRD.manifest, version: "0.1.1" } };
        engine.sheet(CLERIC, [newer], "en");
