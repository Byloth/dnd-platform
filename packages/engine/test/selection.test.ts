...
import { derive, loadPackages, stableStringify, validate } from "../src/index.js";
import type { PackageSource, Selection } from "../src/index.js";
import { MINI, character, cls, feature, item, makeManifest, miniPackage, readPackage, spell } from "./helpers.js";
...
const ELF = `${MINI}.species.elf`;
const HIGH_ELF = `${MINI}.species.elf.high-elf`;
const KEEN = `${MINI}.feature.elf.keen-senses`;
const CANTRIP = `${MINI}.feature.elf.high-elf.cantrip`;
const ELF_FEAT = `${MINI}.feat.elven-accuracy`;
const LUCKY = `${MINI}.feat.lucky`;
const LIST = `${MINI}.spell-list.wizard`;
const SHIELD = `${MINI}.spell.shield`;
const SLEEP = `${MINI}.spell.sleep`;
const SWORD = `${MINI}.item.longsword`;
const FLAME = `${MINI}.item.flame-tongue`;
const SLAYER = `${MINI}.subclass.fighter.giant-slayer`;

function world(): PackageSource
{
...
function load(selection: Selection, sources: readonly PackageSource[] = [world()]): ReturnType<typeof loadPackages>
{
    return loadPackages(sources, { selection: selection });
}
const inactiveIds = (set: ReturnType<typeof loadPackages>): string[] => set.cascade.inactive.map((e) => e.id);
