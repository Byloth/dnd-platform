...
- **Spell components spelled out** (owner, 2026-09-22): `components.v/s/m` are the books' notation, not the format's naming rule; v1 renames them to `verbal`, `somatic`, `material`, and folds `m` (boolean or text), `cost` and `consumed` into one `material: { text, cost?, consumed? }` object. The engine never reads the field; the migration is a script over the spell files plus one line in the import map.
- **Additive v0 extension for the creation wizard (M1.4, owner 2026-09-24, no migration):** `ruleset.abilityScores` (`standardArray`, `pointBuy: { budget, costs }`, the keys of `costs` being the scores allowed); `item.contents: [{ item, quantity? }]` for packs and kits; `state.currency: { copper, silver, electrum, gold, platinum }`; `ChoiceView.filter` in the engine's output. srd51 0.2.0 fills them: the standard array and point buy come from SRD 5.2 (CC-BY-4.0, credited), since SRD 5.1 has neither. `choices.abilityScores.bonuses` is the player's own adjustment of a score, shown as its own line; adjusting other values (armour class, hit points…) is a later candidate.
- **Additive v0 extension for the wizard's choices (M1.4d, owner 2026-09-24):** `ruleset.languages: [{ id, name, exotic? }]`, the languages a character may learn; srd51 0.3.0 fills it from the 5e-database's SRD languages (8 standard, 8 exotic), written by the import into the hand-authored ruleset. A ruleset without it lets the player write languages by hand. `ChoiceView.optionDetails` in the engine's output names and explains inline options.
- **Item category tags for equipment filters (M1.4d2, srd51 0.4.0):** the import tags items with the 5e-database equipment categories that starting-equipment filters name (holy symbols, arcane and druidic foci, musical instruments, melee weapons), and a filter's `category` is always such a tag; the engine's `matchesItemFilter` is the rule. No format change. A filter "two martial weapons" (`quantity: 2`) currently gives two of the one weapon picked; picking two different ones is a wizard improvement for later.
- **`open-choice.amount` for ability choices (M1.4, 2026-09-24, additive v0):** a choice `of: ability` with `amount` raises each chosen score by it (a modify of the feature's own). It serves the variant human (phb14) and the half-elf, whose two +1 srd51 had not modelled until 0.5.0. Fixtures that entered those +1 by hand in `bonuses` now answer the choice instead.
- **`ruleset.alignments` for the wizard's personality step (M1.4d3, owner 2026-09-25, additive v0):** `[{ id, name, abbreviation?, text? }]`; srd51 0.6.0 fills it with the nine alignments of the 5e-database's SRD data, written by the import next to the languages. `choices.alignment` stays a string: an id of the list, or what the player typed when the ruleset has none; the composer names the id. The same release gives the acolyte its ideals, which the import had dropped (the 5e-database keeps an ideal's text in `desc`).
- **Phase 1 additive candidates still planned:** an export document schema (`dnd-platform-export/1`, M1.5); optionally `archetype.recommends.equipment` if the newcomer test asks for it.
- **Currency names spelled out in items** (owner, 2026-09-24): `item.cost.currency` uses `cp`, `sp`, `ep`, `gp`, `pp`, the books' notation; v1 renames them to `copper`, `silver`, `electrum`, `gold`, `platinum`, as `state.currency` already does, together with the spell components above. The migration is a script over the item files plus one line in the import map.
