...
- **Phase 1 additive candidates (planned, docs/phase-1):** a `ruleset.abilityScores` block (`standardArray`, `pointBuy: { budget, costs }`) so the wizard reads the methods from the ruleset (M1.4); an export document schema (`dnd-platform-export/1`, M1.5); optionally `archetype.recommends.equipment` if the newcomer test asks for it. All additive at v0.
