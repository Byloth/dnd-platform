...
3. ~~At Phase 0 close, write the Phase 1 execution plan~~ Owner decision (2026-09-22): the Phase 1 execution plan, with the same structure as this directory, is written at the opening of Phase 1, starting with the decisions DEC-04, DEC-09, DEC-12, DEC-13, DEC-15 and the SPA-vs-Nuxt part of DEC-01. Phase 0 closes on its own criteria.

## Open points

- ~~Whether M0.7 should be pulled before M0.4~~ Moot: M0.7 was built after M0.6, on the complete base package, which made its session fixtures real characters.
- ~~Whether the `phb14-stub` fixture (placeholder text, public) is acceptable~~ Accepted (M0.6): structure and feature names are facts, placeholder text is ours; the stub stays `redistributable: true` so that it can live in the public tree.
- ~~Session estimates~~ Revised by the progress log: every milestone closed in one to two sittings with agent assistance, content authoring included; the estimates of this document were conservative by a factor of two to three.

## Progress log

| Milestone | Closed | Tag | Notes |
|---|---|---|---|
| M0.8 | 2026-09-22 | v0.2.0 | `dnd build` (one canonical JSON bundle per package, the engine's `PackageSource`; private bundles only under `content-private/build/`), `dnd derive` (`--json` byte-equal to the snapshots, `--text` the readable sheet with provenance, `--explain`, package resolution by sibling `packages.yaml` or by id through the content roots), golden `sheet.txt` for three public fixtures and the reference Monk, README CLI section, `CONTRIBUTING.md`. Phase 0 done criteria verified one by one in `00-README.md`; the "second character of every class without warnings" criterion found two engine gaps (choices registered before their level gate held) and an upstream data error (the Use an Object rule named "Search"), both fixed. Risk register reviewed at Phase 0 close. **Phase 0 done.** |
