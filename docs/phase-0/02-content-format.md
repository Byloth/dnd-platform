...
| `open-choice` | `choice`, `of` (`subclass`, `skill`, `spell`, `feat`, `asi-or-feat`, `fighting-style`, `language`, `tool`, `ability`, `equipment`, `option`), `count`, `from?` (ids), `filter?`, `options?` (inline option entities), `level?`, `prerequisites?`, `amount?` (M1.4: for `of: ability`, how much each chosen score rises) | Ask the player something; answers live in the character's `answers`. Inline `options` carry `id`, `name`, `text`, `effects`, `prerequisites`: a chosen option's effects apply (Fighting Style, Metamagic, Draconic Ancestry, Eldritch Invocations, Hunter's Prey). |
