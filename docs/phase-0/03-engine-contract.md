...
interface ComputedSheet {
  ...
  choices: ChoiceView[];               // key '<owner id>#<choice id>', owner, choice, of, count, options, filter (when options is empty: a spell list, a level; M1.4), optionDetails (name and text of inline options; M1.4d), answers, answered, level
