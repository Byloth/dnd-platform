...
- **Published to GitHub Pages** by a workflow of the public repository, on every push to the default branch; the site lives under the repository path (`/dnd/` until the project name is settled), so `app.baseURL` comes from an environment variable with that default, and the generated `404.html` is the single-page fallback for client routes.
...
- **Everything runs in the browser.** The engine is imported as a library; package validation uses the same JSON Schemas and the same Ajv configuration as `dnd validate`, extracted from the CLI into the schema package so both share it; YAML is parsed in the browser with the same `yaml` library the CLI uses.
- **The monorepo build stays `tsc --build` for the library packages;** the web package is excluded from the root project references and has its own scripts (`nuxt typecheck`, `nuxt generate`, `vitest` with the Nuxt environment), called from root scripts prefixed `web:`. Root `lint` covers it through an ESLint block scoped to `packages/web/**` that spreads `@byloth/eslint-config-nuxt`. The root Vitest configuration becomes a projects configuration: the Node project for engine, schema, cli and composer; the Nuxt project for web.
...
- `pnpm web:generate` = `dnd build` for the SRD, copy of the bundle into `public/content/`, `nuxt generate`. Output `.output/public/` with `index.html`, `200.html`, `404.html`, hashed assets, `content/srd51.json`.
