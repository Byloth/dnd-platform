...
- **Locale conversions are display-only**: speeds and ranges stay in feet in content and in the computed sheet. ~~The Italian interface shows metres beside feet (1,5 m per 5 ft) in the sheet and in print, never in exports.~~ Replaced by the owner on 2026-09-25: a units setting, imperial or metric, defaulting from the language (metric in Italian), converted with the Italian manuals' convention (5 ft = 1,5 m, 1 lb = 0,5 kg), never in exports ([09-interface-revisions.md](09-interface-revisions.md)).
- **Done on 2026-09-25** ([11-italian-content.md](11-italian-content.md), "As built"): the official Italian SRD 5.1 was found online (CC BY 4.0) and the owner provided the Italian Player's Handbook; srd51-it (public) and phb14-it (private) translate everything, one package per language and per book, loaded by the interface's language, which follows the browser's.
- **Amended by the owner on 2026-09-25** (superseded the same day by the line above): the official Italian SRD PDF is not available, so the translation is drafted from the English with the glossary and online references for the terms, each recorded; no OCR step. The translation depends on srd51 by id, not at the exact version, with a coverage check. Its first packet (what the wizard shows) moves to M1.4r.
