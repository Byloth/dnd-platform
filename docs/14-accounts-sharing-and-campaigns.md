...
- **Allowed packages**: the list of packages a character in this campaign may use, each pinned to a version. Characters in a campaign are validated against this list; a character built elsewhere shows warnings on join until the DM allows the missing packages or the player removes the choices.
